...
import { updatePairDayData, updatePairHourData, updateTokenDayData, updateElkDexDayData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = ElkDexFactory.load(FACTORY_ADDRESS)!
...
export function handleSwap(event: Swap): void {
  ...
  let elkDexDayData = updateElkDexDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  elkDexDayData.dailyVolumeUSD = elkDexDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  elkDexDayData.dailyVolumeETH = elkDexDayData.dailyVolumeETH.plus(trackedAmountETH)
  elkDexDayData.dailyVolumeUntracked = elkDexDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  elkDexDayData.save()
