...
export function handleSync(event: Sync): void {
  try {
    // Place the entire function logic here as before
    let pair = Pair.load(event.address.toHex());
    if (pair === null) throw new Error("Failed to load Pair");

    let token0 = Token.load(pair.token0);
    let token1 = Token.load(pair.token1);
    if (token0 === null || token1 === null) throw new Error("Failed to load Tokens");

    let elkdex = ElkFactory.load(FACTORY_ADDRESS);
    if (elkdex === null) throw new Error("Failed to load ElkFactory");
    ...
    if (bundle === null) throw new Error("Failed to load Bundle");
    ...
    if (bundle.ethPrice.notEqual(ZERO_BD)) {
      trackedLiquidityETH = getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(bundle.ethPrice);
    } else {
    ...
    // Log the error without retrying or rewinding, and return
    log.warning("Skipping block due to error at block {}: {}", [
